...
const statuses = ["ACTIVE", "ACTIVE", "ACTIVE", "ACTIVE", "INACTIVE", "LAPSED", "PENDING"] as const;
const donorTypes = ["INDIVIDUAL", "INDIVIDUAL", "INDIVIDUAL", "CORPORATE", "FOUNDATION"] as const;
const channels = ["EMAIL", "PHONE", "MAIL", "SMS"] as const;
const frequencies = ["MONTHLY", "QUARTERLY", "YEARLY", "BIWEEKLY"] as const;
...
function generateDonations(): { total: number; count: number; average: number; highest: number; lastDate: Date | null; firstDate: Date | null } {
  ...
  for (let i = 0; i < count; i++) {
    // Génère des montants réalistes (25$ à 5000$)
  ...
  // Date du dernier don (dans les 2 dernières années)
...
async function main() {
  ...
  // Supprimer les données existantes dans le bon ordre (relations d'abord)
  ...
  for (let i = 0; i < 50; i++) {
    ...
    donors.push(donor);
    console.log(`✅ Donateur ${i + 1}/50: ${firstName} ${lastName} (${location.city})`);
  }

  console.log("\n🎉 Seeding terminé avec succès!");
  console.log(`   📊 ${donors.length} donateurs créés`);
  
  // Statistiques
  const activeCount = donors.filter(d => d.status === "ACTIVE").length;
  const totalDonations = donors.reduce((sum, d) => sum + d.totalDonations, 0);
  
  console.log(`   👥 ${activeCount} donateurs actifs`);
