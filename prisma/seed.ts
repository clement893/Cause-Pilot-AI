...
const paymentMethods = ["CREDIT_CARD", "DEBIT", "BANK_TRANSFER", "CHECK", "CASH", "PAYPAL"];
...
async function main() {
  ...
  for (let i = 0; i < donorCount; i++) {
    ...
    for (let j = 0; j < numDonations; j++) {
      ...
      donationData.push({
        amount: randomInt(1, 100) * 10 + randomInt(0, 9), // 10$ à 1009$
    ...
    const donor = await prisma.donor.create({
      data: {
        ...
        country: "Canada",
        status: randomElement(["ACTIVE", "ACTIVE", "ACTIVE", "INACTIVE", "LAPSED"]),
        donorType: randomElement(["INDIVIDUAL", "INDIVIDUAL", "INDIVIDUAL", "CORPORATE", "FOUNDATION"]),
    ...
    if ((i + 1) % 10 === 0) {
      console.log(`  Created ${i + 1}/${donorCount} donors...`);
    }
  }
  
  console.log(`✅ Created ${donorCount} donors with donations`);
  ...
  console.log("\n📊 Seed completed!");
  console.log(`   - ${totalDonors} donors`);
  console.log(`   - ${totalDonations} donations`);
  console.log(`   - ${totalAmount._sum.amount?.toLocaleString("fr-CA", { style: "currency", currency: "CAD" })} total`);
}
