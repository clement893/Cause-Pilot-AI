...
// Usage: npx tsx scripts/create-invitation-table.ts
...
const prisma = new PrismaClient({
  datasources: {
    db: {
      url: process.env.DIRECT_DATABASE_URL || process.env.DATABASE_URL || 'postgresql://postgres:DSCeLISPbWoLYHDubmnLMEXLXrDQdgYl@hopper.proxy.rlwy.net:10280/railway',
    },
  },
});
...
async function createTable() {
  try {
    ...
    const result = await prisma.$queryRawUnsafe<Array<{ count: bigint }>>('SELECT COUNT(*) as count FROM "AdminInvitation"');
    const count = result[0]?.count || 0n;
    console.log(`✅ Table AdminInvitation créée avec succès! (${count} invitations)`);
