/**
 * Script pour configurer la base de données de la Fondation des Amis
 */
...
const FONDATION_AMIS_DB_URL = 'postgresql://postgres:HeEtgJunuApzNoUykzksQXfltumYRjid@maglev.proxy.rlwy.net:33476/railway';

const prisma = new PrismaClient({
  datasources: { db: { url: MAIN_DATABASE_URL } },
});

async function setupFondationAmis() {
  console.log('🔧 Configuration de la base de données pour Fondation des Amis\n');

  try {
    // 1. Trouver l'organisation Fondation des Amis
    const organizations = await prisma.organization.findMany({
      where: {
        OR: [
          { name: { contains: 'des Amis', mode: 'insensitive' } },
          { slug: { contains: 'fondation-des-amis', mode: 'insensitive' } },
          { slug: { equals: 'test-2' } },
        ],
      },
    ...
    if (organizations.length === 0) {
      console.log('❌ Organisation "Fondation des Amis" non trouvée');
    ...
    const testPrisma = new PrismaClient({
      datasources: { db: { url: FONDATION_AMIS_DB_URL } },
    });
    ...
    try {
      ...
      const tableCount = await testPrisma.$queryRawUnsafe<Array<{ count: bigint }>>(
    ...
    console.log(`      DATABASE_URL="${FONDATION_AMIS_DB_URL}" npx prisma migrate deploy`);
    console.log('');
    console.log('   2. Migrer les données existantes (si nécessaire):');
    console.log(`      DATABASE_URL="${MAIN_DATABASE_URL}" npx tsx scripts/migrate-org-data.ts ${org.id} "${FONDATION_AMIS_DB_URL}"`);
    console.log('');
